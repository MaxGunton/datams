...
def deployment_format(df, compute=None):
    ...
    if 'oids' in df.columns:
        df['oids'] = df['oids'].apply(
            lambda x: [] if pd.isna(x) else [int(i) for i in x.split(',')]
        )
...
def mooring_format(df, compute=None):
    ...
    if meets_requirements(compute, 'deployment', ['dep_reg', 'dep_cou', 'dep_orgs'],
        ...
        def combine_cols(row):
            all_parts = []
            if not pd.isna(row['dep_orgs']):
                all_parts.append(f"[{row['dep_orgs']}]")
            location = []
            if not pd.isna(row['dep_reg']):
                location.append(row['dep_reg'])
            if not pd.isna(row['dep_cou']):
                location.append(row['dep_cou'])
            all_parts.append(', '.join(location))
            return ' -- '.join(all_parts)

        df['deployment'] = df.apply(combine_cols, axis=1)
