...
function formatMap(id, center, zoom, points) {
  let map = L.map(id).setView(center, zoom);
    L.tileLayer('https://api.maptiler.com/maps/ocean/{z}/{x}/{y}.png?key=2jroBbfSaj5lpAcKIUhs', {
  ...
  for (let i = 0; i < points.length; i++) {
    ...
    paths[i].setAttribute("fill", points[i].get("color"));
    paths[i].setAttribute("stroke", "black");
    paths[i].setAttribute("stroke-width", "1");
    paths[i].setAttribute("d", points[i].get("icon"));
    icons[i] = L.divIcon({
      className: "div-icon",
      html: "<svg>" + paths[i].outerHTML + "</svg>",
      iconSize: [20, 32],
      iconAnchor: [12, -12],
      popupAnchor: [0, 0],
    });
    markers[i] = L.marker(
      [points[i].get("latitude"), points[i].get("longitude")],
      {
        icon: icons[i],
    ...
    markers[i].bindPopup(points[i].get("html"));
  }
}
